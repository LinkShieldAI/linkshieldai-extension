...
const recentSiteStatus = new Map();
...
const cacheReady = loadScanCache();
...
async function getProtectionSettings() {
    return new Promise(resolve => {
        chrome.storage.sync.get([
            ...
            resolve({
                enableProtection: data.enableProtection !== false,
                showNotifications: data.showNotifications !== false,
                skipSafeSites: data.skipSafeSites !== false,
                showScanScreen: data.showScanScreen === true,
                cacheDurationMs: (data.cacheDuration || 24) * 60 * 60 * 1000,
                scanTimeoutMs: (data.scanTimeout || 5) * 1000
            });
        });
    });
}
...
chrome.webNavigation.onBeforeNavigate.addListener(async (details) => {
    ...
    if (settings.skipSafeSites && isCommonSafeUrl(url)) {
        console.log("LinkShield: Skipping common safe URL:", url);
        setRecentStatus(scanTargetUrl, 'trusted-safe', url);
        return;
    }
    ...
    // Check cache
    const cached = await getCachedResult(scanTargetUrl);
    if (cached) {
    ...
    if (!settings.showScanScreen) {
        console.log("LinkShield: Cache miss - scanning silently");
        runSilentScan(url, scanTargetUrl, details.tabId);
        return;
    }
...
function handleCachedResult(result, url, tabId) {
    const scanTargetUrl = getScanTargetUrl(url);
    setRecentStatus(scanTargetUrl, result === 'safe' ? 'cached-safe' : result, url);
    ...
    switch (result) {
        ...
        case 'nsfw':
            ...
            maybeShowThreatNotification('nsfw', scanTargetUrl);
            chrome.tabs.update(tabId, {
                url: chrome.runtime.getURL(
                    `warning.html?url=${encodeURIComponent(url)}&type=nsfw&tabId=${tabId}&scanTarget=${encodeURIComponent(scanTargetUrl || '')}`
                )
            });
            break;
        case 'malicious':
            updateBadge('blocked', tabId);
            maybeShowThreatNotification('malicious', scanTargetUrl);
            chrome.tabs.update(tabId, {
                url: chrome.runtime.getURL(
                    `warning.html?url=${encodeURIComponent(url)}&type=malicious&tabId=${tabId}&scanTarget=${encodeURIComponent(scanTargetUrl || '')}`
                )
            });
            break;
    }
...
async function maybeShowThreatNotification(result, scanTargetUrl) {
    const settings = await getProtectionSettings();
    if (!settings.showNotifications) return;

    showNotification(
        result === 'malicious' ? 'Dangerous site detected' : 'NSFW site detected',
        `${scanTargetUrl} was flagged by LinkShield.`
    );
}

async function runSilentScan(url, scanTargetUrl, tabId, force = false) {
    if (pendingScans.has(scanTargetUrl)) {
        setRecentStatus(scanTargetUrl, 'scanning', url);
        return pendingScans.get(scanTargetUrl);
    }
    ...
    const scanPromise = (async () => {
        try {
            ...
            if (!apiKey) {
                updateBadge('no-key', tabId);
                setRecentStatus(scanTargetUrl, 'not-scanned', url);
            ...
            if (!force) {
                const cached = await getCachedResult(scanTargetUrl);
                if (cached) {
                    handleCachedResult(cached.result, url, tabId);
                    return cached;
                }
            }

            if (isRateLimited()) {
                updateBadge('rate-limit', tabId);
                setRecentStatus(scanTargetUrl, 'not-scanned', url);
            ...
            if (scanResult.result !== 'error') {
                await cacheResult(scanTargetUrl, scanResult.result);
            }

            if (scanResult.result === 'safe') {
                updateBadge('safe', tabId);
                setRecentStatus(scanTargetUrl, 'cached-safe', url);
                logActivity(url, scanTargetUrl, 'safe', 'silent_scan');
                return scanResult;
            }

            if (scanResult.result === 'malicious' || scanResult.result === 'nsfw') {
                if (scanResult.result === 'malicious') {
                    logBlockedSite(url, scanTargetUrl);
                    updateBadge('blocked', tabId);
                } else {
                    logNsfwSite(url, scanTargetUrl);
                    updateBadge('nsfw', tabId);
                }
                ...
                if (settings.showNotifications) {
                    showNotification(
                        scanResult.result === 'malicious' ? 'Dangerous site detected' : 'NSFW site detected',
                        `${scanTargetUrl} was flagged by LinkShield.`
                    );
                }

                setRecentStatus(scanTargetUrl, scanResult.result, url);
                chrome.tabs.update(tabId, {
                    url: chrome.runtime.getURL(
                        `warning.html?url=${encodeURIComponent(url)}&type=${scanResult.result}&tabId=${tabId}&scanTarget=${encodeURIComponent(scanTargetUrl)}&silent=1`
                    )
                });
                return scanResult;
            }
            ...
            console.error('LinkShield: Silent scan failed:', error);
            updateBadge('error', tabId);
            setRecentStatus(scanTargetUrl, 'not-scanned', url);
            return { result: 'error', error: error.message };
        } finally {
            pendingScans.delete(scanTargetUrl);
        }
    })();

    pendingScans.set(scanTargetUrl, scanPromise);
    return scanPromise;
}
...
async function scanUrlWithTimeout(url, apiKey) {
    ...
    const settings = await getProtectionSettings();
    const timeoutId = setTimeout(() => controller.abort(), settings.scanTimeoutMs);
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            console.error(`LinkShield: Malicious scan timeout after ${settings.scanTimeoutMs}ms`);
        }
        return 'error';
    } finally {
...
async function scanUrlForNsfwWithTimeout(url, apiKey) {
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            console.error(`LinkShield: NSFW scan timeout after ${settings.scanTimeoutMs}ms`);
        }
        return 'error';
    } finally {
        clearTimeout(timeoutId);
...
async function loadScanCache() {
    return new Promise(resolve => {
        chrome.storage.local.get([STORAGE_KEYS.SCAN_CACHE], (data) => {
            const storedCache = data[STORAGE_KEYS.SCAN_CACHE] || {};
            scanCache.clear();

            for (const [url, cached] of Object.entries(storedCache)) {
                if (cached && cached.result && cached.timestamp) {
                    scanCache.set(url, cached);
                }
            }

            console.log(`LinkShield: Loaded ${scanCache.size} cached scan results`);
            resolve();
        });
    });
}
...
function saveScanCache() {
    const storedCache = Object.fromEntries(scanCache.entries());
    chrome.storage.local.set({ [STORAGE_KEYS.SCAN_CACHE]: storedCache });
}
...
async function getCachedResult(url) {
    ...
    if (age > settings.cacheDurationMs) {
...
function setRecentStatus(scanTargetUrl, status, url) {
    if (!scanTargetUrl) return;
    recentSiteStatus.set(scanTargetUrl, {
        status,
        url,
        timestamp: Date.now()
    });
...
function logActivity(url, scanTargetUrl, type, action = 'scanned') {
    chrome.storage.local.get(['recentActivity'], (data) => {
        const activity = data.recentActivity || [];
        activity.unshift({
            url,
            scanTargetUrl: scanTargetUrl || getScanTargetUrl(url),
            type,
            action,
            timestamp: Date.now()
        });

        chrome.storage.local.set({ recentActivity: activity.slice(0, 100) });
    });
}
 ...
 */
function logBlockedSite(url, scanTargetUrl = getScanTargetUrl(url)) {
    ...
    logActivity(url, scanTargetUrl, 'malicious', 'blocked');
}

/**
 * Log blocked NSFW site
 */
function logNsfwSite(url, scanTargetUrl = getScanTargetUrl(url)) {
    ...
    logActivity(url, scanTargetUrl, 'nsfw', 'blocked');
}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    if (request.action === 'clearCache') {
        scanCache.clear();
        failedScans.clear();
        chrome.storage.local.remove([STORAGE_KEYS.SCAN_CACHE], () => {
    ...
    if (request.action === 'getCurrentSiteStatus') {
        (async () => {
            const url = request.url;
            const scanTargetUrl = getScanTargetUrl(url);
            if (!scanTargetUrl) {
                sendResponse({ status: 'not-scanned', domain: '', url });
                return;
            }

            const isTrusted = await isTrustedSite(url);
            const cached = await getCachedResult(scanTargetUrl);
            const recent = recentSiteStatus.get(scanTargetUrl);
            const settings = await getProtectionSettings();

            let status = recent?.status || 'not-scanned';
            if (pendingScans.has(scanTargetUrl)) status = 'scanning';
            if (cached?.result === 'safe') status = 'cached-safe';
            if (cached?.result === 'malicious' || cached?.result === 'nsfw') status = cached.result;
            if (isTrusted) status = 'trusted';
            if (!settings.enableProtection) status = 'protection-off';

            sendResponse({
                status,
                domain: scanTargetUrl,
                url,
                trusted: isTrusted,
    ...
    if (request.action === 'trustSite' || request.action === 'removeTrust') {
        const scanTargetUrl = getScanTargetUrl(request.url);
        if (!scanTargetUrl) {
            sendResponse({ success: false });
            return;
        }

        if (request.action === 'trustSite') {
            chrome.storage.local.set({ [scanTargetUrl]: true }, () => {
                setRecentStatus(scanTargetUrl, 'trusted', request.url);
                sendResponse({ success: true, domain: scanTargetUrl });
            });
            return true;
        }

        chrome.storage.local.remove([scanTargetUrl, request.url], () => {
            setRecentStatus(scanTargetUrl, 'not-scanned', request.url);
            sendResponse({ success: true, domain: scanTargetUrl });
        });
        return true;
    }
    ...
    if (request.action === 'scanNow') {
        (async () => {
            ...
            const result = await runSilentScan(request.url, scanTargetUrl, request.tabId, true);
            sendResponse(result);
        })();
        return true;
    }
        ...
        (async () => {
            try {
                ...
                if (scanResult.result === 'malicious') {
                    ...
                    logNsfwSite(url, scanTargetUrl);
                    maybeShowThreatNotification('nsfw', scanTargetUrl);
                } else if (scanResult.result === 'safe') {
                    setRecentStatus(scanTargetUrl, 'cached-safe', url);
                    logActivity(url, scanTargetUrl, 'safe', 'preload_scan');
                }
