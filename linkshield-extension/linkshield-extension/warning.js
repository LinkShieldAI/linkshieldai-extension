...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const scanTargetUrl = params.get('scanTarget') || getScanTargetUrl(targetUrl);
    ...
    displayScannedSite(scanTargetUrl);
    displaySilentScanNote(silentMode);
...
function showSafeResult(url) {
    ...
    // Green checkmark
    shieldIcon.style.background = '#4CAF50';
    shieldIcon.style.boxShadow = 'none';
...
function showMaliciousWarning() {
    ...
    const scanTargetUrl = params.get('scanTarget') || getScanTargetUrl(url);
    
    // Update URL to include type parameter
    const newUrl = new URL(window.location.href);
    newUrl.searchParams.set('type', 'malicious');
    newUrl.searchParams.set('scanTarget', scanTargetUrl || '');
...
function showNoKeyError() {
    ...
    shieldIcon.style.background = '#FF9800';
    shieldIcon.style.animation = 'none';
...
function showRateLimitError(url) {
    ...
    shieldIcon.style.background = '#9E9E9E';
    shieldIcon.style.animation = 'none';
...
function updateWarningUI(type) {
    ...
    if (type === 'nsfw') {
        ...
        title.textContent = 'NSFW Content Warning';
        title.style.color = '#ff9800';
        
        subtitle.textContent = 'LinkShield found adult or sensitive content on this site';
        subtitle.style.color = '#ffb74d';
        ...
        checkboxLabel.textContent = 'Trust this site';
        
    } else {
        // Malicious site warning (default)
        shieldIcon.style.background = '#c83f3f';
        shieldIcon.style.boxShadow = 'none';
        shieldIcon.style.animation = 'none';
        ...
        subtitle.textContent = 'LinkShield found signs this site may be dangerous';
...
function displayScannedSite(scanTargetUrl) {
    const urlElement = document.getElementById('scannedSiteUrl');
    if (!urlElement) return;

    urlElement.textContent = scanTargetUrl || 'Unknown site';
}

function displaySilentScanNote(showNote) {
    const note = document.getElementById('silentScanNote');
    if (!note) return;
...
function logWarningEvent(url, type = 'malicious') {
    ...
    chrome.storage.local.get([historyKey], (data) => {
        ...
        history.unshift({
            url: url,
            scanTargetUrl: getScanTargetUrl(url),
            timestamp: Date.now(),
            type: type,
            action: 'warned'
        });
